...
for epoch in range(50 if args.autoencoder else 100):
    print('epoch %s' % epoch)
    model.train()
    ...
    for i, img in enumerate(tqdm(train_loader)):
        ...
        recon, kl_cost = model(process_input(img_gt))
    ...
    with torch.no_grad():
        ...
        if epoch % 1 == 0:
            ...
            for i, img in enumerate(val_loader):
                ...
                recon, kl_cost = model(process_input(img_inp))
            ...
            if epoch == 0: 
                with open(os.path.join(args.base_dir, 'samples/real.npz'), 'wb') as f: 
                    img_inp = img_inp.cpu().data.numpy()
                    np.save(f, img_inp)
