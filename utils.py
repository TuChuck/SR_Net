...
def preprocess(dataset):
    ...
    # for i in remove:
    #     dataset = np.concatenate([dataset[:i], dataset[i+1:]], axis=0)

    return dataset[:, :, :, ::2] #dataset[:, :, :, :]
...
def load_model_from_file(path, epoch, model='dis'):
    ...
    model_.load_state_dict(torch.load(os.path.join(path, 'models/%s_%d.pth' % (model, epoch))),strict=False)
