...
class NewCRF(nn.Module):
    ...
    def forward(self, x, v):
        ...
        return out
#### Ref. SalsaNext: Fast, Uncertainty-aware Semantic Segmentation of LiDAR Point Clouds for Autonomous Driving 
#### Context Block ####
class ResContextBlock(nn.Module):
    def __init__(self, in_filters, out_filters, padding = None):
        ...
        self.conv2 = nn.Conv2d(out_filters, out_filters, (3,3), padding=padding[0])
        self.act2 = nn.LeakyReLU()
        self.bn1 = nn.BatchNorm2d(out_filters)

        self.conv3 = nn.Conv2d(out_filters, out_filters, (3,3),dilation=2, padding=padding[1])
...
class ResBlock(nn.Module):
    def __init__(self, in_filters, out_filters, dropout_rate, kernel_size=(3, 3), stride=1,
        ...
        if pooling:
            ...
            self.pool = nn.AvgPool2d(kernel_size=kernel_size, stride=(1,2), padding=(0,1))
        else:
            self.dropout = nn.Dropout2d(p=dropout_rate)
...
class CRF_based_LSRNet(nn.Module):
    def __init__(self, input_dim = 3):
        super(CRF_based_LSRNet, self).__init__()
        
        self.cntModule1 = ResContextBlock(input_dim,32)
        self.cntModule2 = ResContextBlock(32,32)
        # self.cntModule3 = ResContextBlock(32,32)

        self.resBlock1 = ResBlock(32, 2 * 32, 0.2, kernel_size=(1, 3) ,pooling=True)
        self.resBlock2 = ResBlock(2 * 32, 2 * 2 * 32, 0.2, pooling=False)
        self.resBlock3 = ResBlock(2 * 2 * 32, 2 * 4 * 32, 0.2, pooling=False)
        # self.resBlock4 = ResBlock(2 * 4 * 32, 2 * 4 * 32, 0.2, pooling=False)

        self.crf2 = NewCRF(input_dim=256, embed_dim=128, window_size=7, v_dim=64, num_heads=16)
        self.crf1 = NewCRF(input_dim=32, embed_dim=64, window_size=7, v_dim=64, num_heads=8)

        self.disp_head = DispHead(64, input_dim)

        self.TransposedConv = nn.ConvTranspose2d(32, 64,kernel_size=(3,3),stride=(2,1),padding=(1,1),output_padding=(1,0))
        self.LeakyReLU = nn.LeakyReLU()
    
    def forward(self,x):
        Bilinear_x = upsample(x, scale_factor = (2,1))

        Cntx = self.cntModule1(x)
        Cntx = self.cntModule2(Cntx)
        # Cntx = self.cntModule3(Cntx)        
        
        ## first path 
        TrasConv = self.LeakyReLU(self.TransposedConv(Cntx))

        ## second path
        embedding_x_1 = self.resBlock1(Cntx)

        embedding_x = self.resBlock2(embedding_x_1[0])
        embedding_x = self.resBlock3(embedding_x)
        # embedding_x = self.resBlock4(embedding_x)

        e2 = self.crf2(embedding_x, embedding_x_1[1][:,:,:,0::2])
        e2 = nn.PixelShuffle(2)(e2)

        e1 = self.crf1(e2, TrasConv)       
        high_res = self.disp_head(e1, 0)

        output = Bilinear_x + high_res 
