...
class UpBlock(nn.Module):
    ...
    def forward(self, x, skip, is_upsampling = True):
        if is_upsampling:
...
class SalsaNext(nn.Module):
    def __init__(self, nclasses, upsampling_factor = 2):
        ...
        self.downCntx = ResContextBlock(32, 32)   #### H9 ######
        #self.downCntx = ResContextBlock(6, 32)   #### H9 ######
    ...
    def forward(self, x):
        ...
        for _ in range(self.upsampling_factor):
            up_x = self.Up_block[_](up_x)

        downCntx = self.downCntx(up_x)
...
if __name__=='__main__':
    x = torch.rand((16,3,10,256))
    # x = torch.rand((16,5,64,1024))
    upsampling_factor = 2

    model = SalsaNext(3, upsampling_factor = upsampling_factor)
